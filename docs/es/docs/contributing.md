...
Primero, puedes revisar las guías para [ayuda a FastAPI y obten ayuda](help-fastapi.md){.internal-link target=_blank}.

# Desarrollando

Si ya has clonado el  <a href="https://github.com/fastapi/fastapi" class="external-link" target="_blank">repo de fastapi</a> y quieres profundizar mas en el código, aqui hay algunas guías para preparar tu entorno.

### Virtual environment con `venv`

Puedes crear un entorno virtual local en un directorio usando el modulo `venv` de Python. Ahora vamos a hacer esto en el repo clonado (donde estan los `requirements.txt`):
...
Esto creará el directorio `./env/` con los binarios de Python, entonces podrás instalar paquetes en tu entorno local.
...
Si te muestra que el el binario de `pip` se encuentra ubicado en `env/bin/pip` es porque todo funcionó. 🎉

Asegurate que la última versión de pip en tu entorno local virtual para evitar errores en los siguientes pasos:
...
Esto te mantendrá seguro de que si usas un programa de la terminal instalado por ese paquete, es porque usarás el paquete del entorno local, no el paquete del entorno global.
...
Despues de activar el entorno como se explica en pasos anteriores:
...
Este comando instalá todas las dependencias y tu FastAPI local en tu entorno local.

### Usando FastAPI en tu entorno local

Si creas un archivo de python que importe y use FastAPI, y lo ejecutes con el Python de tu entorno local, este usará el FastAPI que clonaste y que ahora tienes el código fuente.
...
Durante el desarrollo local, puedes ejecutar un script que construye el sitio y comprueba si hay cambios, recargando en vivo:
...
Pero también puedes usar <a href="https://typer.tiangolo.com/typer-cli/" class="external-link" target="_blank">Typer CLI</a>, y obtendrás autocompletado en tu terminal para los comandos después de instalar la autocompletación.
...
Además, hay herramientas/scripts adicionales en el lugar para manejar las traducciones en `./scripts/docs.py`.

/// tip

Tu no necesitas ver el código en `./scripts/docs.py`, solo lo usas en la línea de comandos.
...
Si tu ejecutas los ejemplos, por ejemplo:
...
como Uvicorn por defecto usa el puerto `8000`, la documentación estará en el puerto `8008` no entrará en conflicto.

### Traducciones
La ayuda con las traducciones son MUY apreciadas! Y no se puedría hacer sin la ayuda de la comunidad. 🌎 🚀

Aqui están los pasos para ayudar con las traducciones.
...
* Revisa esos pull requests, solicitando cambios o aprobandolos. Para los lenguajes que no hablo, esperaré a que los otros revisen las traducciones antes de realizar el merge.
...
* Verifica en <a href="https://github.com/fastapi/fastapi/discussions/categories/translations" class="external-link" target="_blank">GitHub Discussion</a> para coordinar traducciones en tu idioma. Puedes suscribirte a él, y cuando haya un nuevo pull request para revisar, un comentario automático será añadido a la discución.

* Si traduces páginas, agregar un único pull request por cada página traducida. Esto hará mucho mas fácil de revisar por otros.
...
Ahora ejecuta el live serrver para los documentos en español:
...
De otra manera, puedes hacer los pasos de los scripts manualmente.
...
Si vas a tu navegador vas a ver que ahora la documentación se muestra en tu nueva sección (el cuadro de informacón en la parte superior ha desaparecido). 🎉
...
Ese comnado también creó un archivo "dummy" `docs/ht/index.md` para la página principal, puedes comenzar traduciendo esa.
...
Como ya se mencinó arriba, puedes user el `./scripts/docs.py` con el comando `live` para previsualizar los resultados (o `mkdocs serve`).
...
```console
// Usa el comando "build-all", esto tomará un poco
...
* Trauce solo la documentación en Markdown (`.md`). No traducir los códigos de ejemplo ubicados en `./docs_src`.
...
* No modifiques las rutas de las imagenes, archivos de código, documentación en Markdown.
...
* Sin embargo, cuando un documento en Markdown es traducido, el `#hash-parts` los links a sus headings pueden cambiar. Actualiza esos links si es posible.
    ...
    * Cuando estás traduciendo un documento, no pre-traduzcas los `#hash-parts` ese link hacia los headings que no tengan la documentación traducida.
...
Este comando genera un directorio `./htmlcov/`, si abres el archivo `./htmlcov/index.html` en tu navegador, puedes explorar interactivamente las regiones de código que están cubiertas por los tests, y puedas notar si hay alguna region que falta.
